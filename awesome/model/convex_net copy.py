...
# Global variable to control activation function
...
class ConvexNet(nn.Module):
    ...
    def forward(self, x):
        x_input = x
        x = apply_activation(self.W0y(x))
        x = apply_activation(self.W1z(x) + self.W1y(x_input))
...
class WNSkipBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_input: torch.Tensor):
        return apply_activation(self.ln(x) + self.skp(x_input))
...
class OutBlock(SkipBlock):
    def __init__(self,
                 ...
                 in_skip_features: int = 2,
                 **kwargs) -> None:
        super().__init__(in_features, out_features, in_skip_features)
...
class ConvexNextNet(nn.Module):
    ...
    def forward(self, x):
        x_input = x
        x = apply_activation(self.input(x))
    ...
    def enforce_convexity(self) -> None:
        with torch.no_grad():
            self.input.weight.data = F.relu(self.input.weight.data)
            for block in self.skip:
                block.ln.weight.data = F.relu(block.ln.weight.data)
                block.skp.weight.data = F.relu(block.skp.weight.data)
            self.out.ln.weight.data = F.relu(self.out.ln.weight.data)
            self.out.skp.weight.data = F.relu(self.out.skp.weight.data)
